 ...
 */
public class UnionFindQU_Rank_PS implements UnionFind {
    private int[] container;
    private int[] heights;

    public UnionFindQU_Rank_PS(int capacity) {
        if (capacity < 0) {
    ...
    public int find(int v) {
        while (v != container[v]) {
            int parent = container[v];
            container[v] = container[container[parent]];
            v = parent;
        }
        return v;
    }
